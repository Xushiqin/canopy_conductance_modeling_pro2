...
FIG_FILE = FIG_DIR / "DD_deltaKGE_vs_dryness_index_biome.jpg"
STAT_FILE = STAT_DIR / "DD_deltaKGE_vs_dryness_index_biome.csv"
PAIRED_FILE = STAT_DIR / "DD_deltaKGE_paired_valid_sites.csv"
BIOME_REG_STAT_FILE = STAT_DIR / "DD_deltaKGE_vs_dryness_index_biome_linear_CI_regression_stats.csv"
GLOBAL_REG_STAT_FILE = STAT_DIR / "DD_deltaKGE_vs_dryness_index_global_linear_CI_regression_stats.csv"
...
def add_delta_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for cfg in MODEL_CONFIGS:
        ...
        print(f"[INFO] {cfg['name']}: paired valid sites for deltaKGE = {int(valid_pair.sum())}")
...
def collect_biome_regression_statistics(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
...
def collect_global_regression_statistics(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
...
def collect_paired_valid_results(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
...
def plot_panel(ax, panel_df: pd.DataFrame, cfg: dict, add_legend: bool = False):
    ...
    # overall regression for all sites: black line + light gray CI
    if len(panel_df) > 1:
        try:
    ...
    # separate linear regression for each biome + matching CI
    for group in GROUP_ORDER:
        sub = panel_df.loc[panel_df["group_label"] == group].copy()
        if len(sub) <= 1:
            continue
    ...
    # panel label outside top-left of axes
    ax.text(
        0, 1.04,
        cfg["panel"],
...
def get_global_y_limits(df: pd.DataFrame):
    y_values = []
    for cfg in MODEL_CONFIGS:
        panel_df = prepare_panel_data(df, cfg["ycol_new"])
...
def print_biome_counts(df: pd.DataFrame):
    print("\n[INFO] Valid plotted rows by model and biome:")
    out_rows = []
    for cfg in MODEL_CONFIGS:
        panel_df = prepare_panel_data(df, cfg["ycol_new"])
...
def main():
    ...
    stat_df = collect_statistics(df)
    stat_df.to_csv(STAT_FILE, index=False)
    print(f"[OK] Statistics saved to: {STAT_FILE}")

    paired_df = collect_paired_valid_results(df)
    paired_df.to_csv(PAIRED_FILE, index=False)
    print(f"[OK] Paired valid-site table saved to: {PAIRED_FILE}")

    biome_reg_df = collect_biome_regression_statistics(df)
    biome_reg_df.to_csv(BIOME_REG_STAT_FILE, index=False)
    print(f"[OK] Biome regression statistics saved to: {BIOME_REG_STAT_FILE}")

    global_reg_df = collect_global_regression_statistics(df)
    global_reg_df.to_csv(GLOBAL_REG_STAT_FILE, index=False)
    print(f"[OK] Global regression statistics saved to: {GLOBAL_REG_STAT_FILE}")

    print_biome_counts(df)

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    axes = axes.flatten()

    y_low, y_high = get_global_y_limits(df)
