...
FIG_FILE = FIG_DIR / "DD_gc_ET_barplot_median_n.jpg"
STAT_FILE = STAT_DIR / "DD_gc_ET_barplot_median_n_summary.csv"
...
def add_n_labels(ax, bars, n_values):
    ...
    for row_idx, metric in enumerate(METRICS):
        ...
        for col_idx, dataset_name in enumerate(["gc", "ET"]):
            ...
            for pair in MODEL_PAIRS:
                ...
                hatch_styles.extend(["", "///"])   # baseline empty, constrained hatched
            ...
            if row_idx == 3:
                ax.set_xticklabels(xlabels, rotation=90, ha="center", va="top", fontsize=9)
            else:
            ...
            if row_idx == 0:
                if dataset_name == "gc":
                    ax.set_title(r"$g_c$", fontsize=13)   # italic g, subscript c
                else:
                    ax.set_title("ET", fontsize=13)
            ...
            add_n_labels(ax, bars, n_values)

            if metric_name in ["KGE'", "Correlation"]:
                for ref in [0.0, 0.5, 0.75]:
                    ax.axhline(ref, linestyle=":", linewidth=0.8, color="gray", zorder=0)
            else:
                ax.axhline(1.0, linestyle=":", linewidth=0.8, color="gray", zorder=0)

            if row_idx == 0 and col_idx == 0:
                legend_handles = [
                    Patch(facecolor="white", edgecolor="black", hatch="", label="Baseline"),
                    Patch(facecolor="white", edgecolor="black", hatch="///", label="Constrained"),
                ]
                ax.legend(
...
def main():
    ...
    gs_df = exclude_igbp_classes(gs_df, exclude_classes=["WET", "CVM"])
    et_df = exclude_igbp_classes(et_df, exclude_classes=["WET", "CVM"])
